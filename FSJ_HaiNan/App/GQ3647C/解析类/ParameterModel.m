...
- (NSString *)getResult:(ParameterModel *)model{
    NSString *str =[NSString stringWithFormat:@"%@解析错误 value=%@ length=%@",model.name,model.parameterValue,model.len];
    NSMutableString *mutstr = [NSMutableString string];
//    V_STRING	字符串类型	根据len参数定义	-
    if ([model.type isEqualToString:@"V_STRING"] && [model.detail isEqualToString:@"-"]) {
    ...
    if ([model.type isEqualToString:@"V_STRING"] && [model.detail isEqualToString:@"字符串"]) {
        ...
        NSString *V_STRING = [[NSString alloc]initWithData:data encoding:enc];
...
//    V_FLOAT	单精度浮点数	固定4字节
    if ([model.type isEqualToString:@"V_FLOAT"]) {
        ...
        {
            VVDLog(@"float转换错误 参数== %@  value ==%@ name == %@",model.parno,model.parameterValue,model.name);
        }
    }
//    V_ENUM_BIT	枚举位	可变根据len参数定义	枚举位的定义，根据返回16进制转2进制确定每个的参数值
    if ([model.type isEqualToString:@"V_ENUM_BIT"]) {
        ...
        for (int i = 1; i < 1<<num; i++) {
            ...
            if (sum == result) {
                //返回序号
                NSMutableString *V_ENUM_BIT = [NSMutableString string];
                for (NSString *tempstr  in mutArr) {
                    [V_ENUM_BIT appendString:[NSString stringWithFormat:@" %@ ",tempstr]];
                }
                return V_ENUM_BIT;
            }
        }
